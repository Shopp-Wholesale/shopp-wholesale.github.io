// script.js — FINAL (Safe cart migration + docId-based cart + stable UI + Location lock + Admin bypass)
...
(function attachModalControls() {
      ...
      try {
        ...
        if (snap.empty) {
          ...
          snap.forEach(doc => {
            ...
            row.innerHTML = `<div style="font-weight:600">${name} • ₹${money(total)}</div>
  ...
  if (closeOrdersBtn) closeOrdersBtn.addEventListener('click', (e) => { e.preventDefault(); if (ordersModal) ordersModal.classList.add('hidden'); });
